...
def delete_realtion_by_id(user_id: int, friend_id: int):
    with Session(engine) as session:
        query1 = session.query(Relationships).filter(
            (Relationships.user_id == user_id) and (Relationships.friend_id == friend_id)).first()
        if query1 is not None:
            session.delete(query1)

        query2 = session.query(Relationships).filter(
            (Relationships.user_id == friend_id) and (Relationships.friend_id == user_id)).first()
...
def get_relationships(user_id: int, status: str):
    with Session(engine) as session:
        ...
        users = session \
            .query(User.username, User.fullname, User.avatar, Relationships.friend_id, Relationships.status,
                   User.online, User.role) \
            .join(Relationships, Relationships.friend_id == User.user_id) \
...
def get_invitation_received(user_id: int):
    with Session(engine) as session:
        invitation_receiveds = session.query(Relationships.user_id).filter(Relationships.friend_id == user_id,
                                                                           Relationships.status == '1').all()
